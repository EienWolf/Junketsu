# Desarmar

> _"Un giro preciso, un crujido metálico... y el arma enemiga cae a tus pies."_

---

## Contenido

- [Descripción](#descripcion)
- [Condiciones Especiales](#condiciones-especiales)
- [Ejemplos de Uso](#ejemplos-de-uso)
- [Preguntas Frecuentes](#preguntas-frecuentes)
- [Estrategias Avanzadas](#estrategias-avanzadas)
- [Acciones Relacionadas](#acciones-relacionadas)

---

## Descripción

**Desarmar** es una técnica avanzada que utiliza un [ataque básico](/basic-attack) para \
despojar al enemigo de su equipo. Combina daño estándar con la posibilidad \
de dejar al objetivo indefenso si logras un [Super Éxito](/great-sucess).

|                 |                                                                   |
| --------------- | ----------------------------------------------------------------- |
| **Costo**       | Costo base del ataque + 1 Stamina                                 |
| **Tipo**        | Acción Primaria (Maniobra)                                        |
| **Exito**       | Daño normal del ataque                                            |
| **Super Exito** | Ademas del daño el objetivo queda [**Desarmado**](/basic-attack). |

---

## Condiciones Especiales

- **Penalizacion**: La tirada se realiza con [**Desventaja**](/disvantage).

---

## 🎮 Ejemplos de Uso

### Escenario 1: Combate Cuerpo a Cuerpo

*"Usas *Estocada rápida* (ataque básico) + 1 Stamina. Tirás 3d6 → 1d6 por penalización. Sacas 6. Defensa enemiga: 0 éxitos.*

- **Resultado**: ¡Superas por 3 éxitos! El hacha del orco sale volando.

### Escenario 2: Uso a Distancia

_"Disparas con tu arco (ataque básico) + 1 Stamina. Tirás 2d6 → 0 dados. Sacas 5. Defensa mago: 1 éxito._

- **Resultado**: Impactas (daño normal), pero no desarmas.

---

## ❓ Preguntas Frecuentes

1. **¿Puedo combinar Desarmar con [Acción Rápida](#link)?**  
   ✅ **No**, pues Desarmar ya es una acción primaria.

2. **¿El enemigo puede contraatacar si fallo el desarme?**  
   ⚠️ **Sí**, el daño normal no bloquea su turno.

3. **¿Funciona contra enemigos con armas encadenadas?**  
   🚫 **No**, requieren [Maniobras Especializadas](#link) no básicas.

4. **¿Puedo usar esto con [Preparar Acción](#link)?**  
   ✅ **Sí**, ej: _"Si el enemigo ataca, Desarmo su espada."_

---

## 🛠️ Estrategias Avanzadas

1. **Combo de Control**

   - **[Derribar](#link)** (turno 1) → **Desarmar** (turno 2).
   - _Enemigos en el suelo tienen -1 éxito de defensa._

2. **Táctica de Distracción**

   - **[Fingir Debilidad](#link)** (dote) → Enemigo se confía → Desarmas con ventaja táctica.

3. **Sinergia Reactiva**
   - Desarma → Enemigo intenta recuperar arma → **[Ataque de Oportunidad](#link)**.

---

## 🔗 Acciones Relacionadas

- **Grupo Primario**: [Atacar](#link), [Maniobras](#link).
- **Sinergias Clave**:
  1. [Ataque de Oportunidad](#link) (cuando recuperan el arma).
  2. [Defensa Mejorada](#link) (protección post-desarme).
  3. [Contragolpe](#link) (para castigar intentos de contraataque).

<!--
Tienes que usar el ataque basico del arma.
El costo de estamina depende de los indicado por el ataque mas 1
Realizas un golpe cuerpo a cuerpo dirigido hacia el agarre del arma del enemigo, buscando que suelte esta misma.
Se tira con desventaja
Super éxito: Consigues desarmar al enemigo.
 -->
